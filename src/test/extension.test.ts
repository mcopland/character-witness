...
test("Ignored characters should not produce diagnostics", async () => {
  await withConfig({ allowedCharacters: ["u+00a3", "u+00a9"] }, async () => {
    ...
    assert.strictEqual(
      diags.length,
      0,
      "Expected 0 diagnostics for ignored characters",
    );
  });
});
...
test("Diagnostic message format should include character and hex code", async () => {
  ...
  assert.ok(
    msg.includes("\u00e9") && msg.includes("00E9"),
    `Message "${msg}" should include the character and its hex code`,
  );
});
...
test("Config | Adding a char to allowedCharacters should suppress diagnostics", async () => {
  ...
  assert.ok(
    diagsBefore.length >= 1,
    `Expected >=1 diagnostic for \u00a9, got ${diagsBefore.length}`,
  );
  ...
  await withConfig({ allowedCharacters: ["u+00a9"] }, async () => {
    ...
    assert.strictEqual(
      diags.length,
      0,
      "Expected 0 diagnostics when \u00a9 is allowed",
    );
  });
});
...
test("Config | Setting enable: false should clear diagnostics; re-enabling should restore them", async () => {
  ...
  assert.ok(
    diags.length >= 1,
    `Expected diagnostics to be restored after re-enable, got ${diags.length}`,
  );
});
...
test("Auto-Replace | buildReplacementEdits should produce edits for mapped characters", async () => {
  ...
  await withConfig(
    ...
    async () => {
      ...
      assert.ok(
        edits.length >= 1,
        `Expected at least 1 edit for em-dash, got ${edits.length}`,
      );
      assert.strictEqual(
        edits[0].newText,
        "-",
        'Expected replacement to be "-"',
      );
    },
  );
});
...
test("Auto-Replace | No edits when autoReplaceOnSave is false", async () => {
  ...
  await withConfig({ autoReplaceOnSave: false }, async () => {
    ...
    assert.strictEqual(
      edits.length,
      0,
      "Expected 0 edits when autoReplaceOnSave is false",
    );
  });
});
  ...
  await withConfig(
    ...
    async () => {
      ...
      assert.strictEqual(
        edits.length,
        0,
        "Expected 0 edits for unmapped character",
      );
    },
  );
});
...
test("Severity | Error-level characters (e.g. NBSP) should produce Error severity", async () => {
  ...
  assert.strictEqual(
    ...
    "Expected Error severity for NBSP",
  );
});
...
test("Severity | Severity overrides should change severity", async () => {
  await withConfig({ severityOverrides: { "u+00a0": "info" } }, async () => {
    ...
    assert.strictEqual(
      diags[0].severity,
      vscode.DiagnosticSeverity.Information,
      "Expected Info severity after override",
    );
  });
});
...
test("Severity | Grouped diagnostic should use worst severity in group", async () => {
  ...
  assert.strictEqual(
    diags.length,
    1,
    `Expected 1 grouped diagnostic, got ${diags.length}`,
  );
  assert.strictEqual(
    diags[0].severity,
    vscode.DiagnosticSeverity.Error,
    "Expected worst severity (Error) in group",
  );
});
...
test("Filtering | includeStrings: false should skip chars in strings", async () => {
  await withConfig({ includeStrings: false }, async () => {
    ...
    assert.strictEqual(
      diags.length,
      0,
      "Expected 0 diagnostics for chars in strings when includeStrings is false",
    );
  });
});
...
test("Filtering | includeComments: false should skip chars in comments", async () => {
  await withConfig({ includeComments: false }, async () => {
    ...
    assert.strictEqual(
      diags.length,
      0,
      "Expected 0 diagnostics for chars in comments when includeComments is false",
    );
  });
});
...
test("Filtering | Chars outside strings/comments should still be flagged even when filters are off", async () => {
  await withConfig(
    { includeStrings: false, includeComments: false },
    async () => {
...
test("Edge | Empty document should produce 0 diagnostics", async () => {
  ...
  assert.strictEqual(
    diags.length,
    0,
    "Expected 0 diagnostics for empty document",
  );
});
...
test("Edge | Newlines-only document should produce 0 diagnostics", async () => {
  ...
  assert.strictEqual(
    diags.length,
    0,
    "Expected 0 diagnostics for newlines-only document",
  );
});

test("Edge | Surrogate pair character (U+1F600) should produce diagnostic with correct code", async () => {
  await withConfig(
    { codePointFormat: "u+", codePointCase: "upper" },
    async () => {
      const { document } = await openDocumentWithContent("\u{1F600}");
      const diags = await waitForDiagnostics(document.uri);
      assert.ok(
        diags.length >= 1,
        "Expected at least 1 diagnostic for surrogate pair",
      );
      assert.ok(
        diags[0].message.includes("U+1F600"),
        `Expected message to include 'U+1F600', got "${diags[0].message}"`,
      );
    },
  );
});
...
test("Edge | Mixed ASCII + non-ASCII on same line should produce grouped diagnostic", async () => {
  await withConfig({ allowedCharacters: [] }, async () => {
    ...
    assert.ok(
      diags[0].message.includes("3 non-ASCII characters"),
      `Expected grouped message, got "${diags[0].message}"`,
    );
  });
});
...
test('Config | codePointFormat "0x" should format code point in message with 0x prefix', async () => {
  await withConfig(
    { codePointFormat: "0x", codePointCase: "lower" },
    async () => {
      const { document } = await openDocumentWithContent("\u00e9");
      const diags = await waitForDiagnostics(document.uri);
      assert.ok(diags.length >= 1);
      assert.ok(
        diags[0].message.includes("0x00e9"),
        `Expected message to include '0x00e9', got "${diags[0].message}"`,
      );
    },
  );
});
...
test('Config | codePointCase "upper" should produce uppercase hex in message', async () => {
  await withConfig(
    ...
    async () => {
      ...
      assert.ok(diags.length >= 1);
      assert.ok(
        diags[0].message.includes("U+00E9"),
        `Expected message to include 'U+00E9', got "${diags[0].message}"`,
      );
    },
  );
});
...
test("Config | diagnosticSeverities [] should suppress all diagnostics", async () => {
  await withConfig({ diagnosticSeverities: [] }, async () => {
    ...
    assert.strictEqual(
      diags.length,
      0,
      "Expected 0 diagnostics when diagnosticSeverities is []",
    );
  });
});
...
test('Config | diagnosticSeverities ["error"] should suppress info-level diagnostics', async () => {
  await withConfig({ diagnosticSeverities: ["error"] }, async () => {
    ...
    assert.strictEqual(
      diags.length,
      0,
      'Expected 0 diagnostics for info-level char with ["error"] filter',
    );
  });
});
  ...
  try {
    ...
    await withConfig({ ignoredPaths: ["**/cw-test-ignored.txt"] }, async () => {
      ...
      assert.strictEqual(
        diags.length,
        0,
        "Expected 0 diagnostics for ignored path",
      );
    });
  } finally {
