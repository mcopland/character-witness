...
async function waitForDiagnostics(uri: vscode.Uri, timeoutMs = 5000, intervalMs = 100): Promise<vscode.Diagnostic[]> {
...
test('10k lines sparse (non-ASCII every 100 lines) should produce diagnostics within 2s', async () => {
  ...
  assert.ok(diags.length >= 100, `Expected >=100 diagnostics, got ${diags.length}`);
  assert.ok(elapsed < 2000, `Expected diagnostics within 2s, took ${elapsed}ms`);
});
...
test('10k lines dense (non-ASCII every line) should produce diagnostics within 3s', async () => {
  ...
  assert.ok(diags.length >= 10000, `Expected >=10000 diagnostics, got ${diags.length}`);
  assert.ok(elapsed < 3000, `Expected diagnostics within 3s, took ${elapsed}ms`);
});
...
test('Cache hit should be faster than cold scan', async () => {
  ...
  assert.ok(diags1.length >= 500, `Expected >=500 diagnostics, got ${diags1.length}`);

  const tempDoc = await vscode.workspace.openTextDocument({ content: 'temp', language: 'plaintext' });
  ...
  console.log(`[perf] Cache test: cold=${coldElapsed}ms, warm=${warmElapsed}ms, diags=${diags2.length}`);
  // Warm path should be at least somewhat faster (generous threshold for CI)
  assert.ok(diags2.length >= 500, `Expected >=500 diagnostics on warm path, got ${diags2.length}`);
});
