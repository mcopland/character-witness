...
function getCachedMatches(
  ...
  if (cached && cached.version === document.version && cached.fingerprint === fingerprint) {
    return cached.matches;
  }

  const matches = findNonAsciiCharacters(document, allowedCharacters, includeStrings, includeComments, document.languageId);
...
function updateEditor(editor: vscode.TextEditor): void {
  try {
    ...
    const matches = getCachedMatches(editor.document, config.allowedCharacters, config.includeStrings, config.includeComments);
